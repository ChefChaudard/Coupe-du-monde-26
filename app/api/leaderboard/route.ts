...
export async function GET(request: Request) {
  ...
  const loadLeaderboard = async (useAdminClient: boolean) => {
    ...
    const [
      ...
      client
        .from("matches")
        .select(
          "id, phase, team_a, team_b, kickoff_at, venue, city, score_a, score_b, is_finished, odds_home, odds_draw, odds_away"
        ),
      fetchAllRows<{
        user_id: string;
        match_key: string;
      ...
      fetchAllRows<{
        ...
        client
          ...
          .select("user_id, group_name, team_name, predicted_position")
  ...
  if (
    ...
    leaderboardData.groupPredictionsError
  ) {
  ...
  const matchesById = new Map(
    (matches ?? []).map((match: { id: number; phase: string; team_a: string; team_b: string; kickoff_at?: string | null; venue?: string | null; city?: string | null; score_a: number | null; score_b: number | null; is_finished: boolean | null; odds_home: number | null; odds_draw: number | null; odds_away: number | null }) =>[match.id, match])
  );
