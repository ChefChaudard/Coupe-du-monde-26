...
type Round32Write = {
  ...
  teamA: string | null;
  teamB: string | null;
  fixture: ReturnType<typeof getRealRound32Fixture> extends infer T ? NonNullable<T> : never;
};

function redirectWithError(message: string): never {
  redirect(`/admin/real-knockout?error=${encodeURIComponent(message)}`);
}

function parseRound32Assignments(value?: string | null) {
  if (!value) return new Map<number, Round32Assignment>();

  try {
    const parsed = JSON.parse(value) as unknown;
    if (!Array.isArray(parsed)) return new Map<number, Round32Assignment>();

    const assignments = new Map<number, Round32Assignment>();

    for (const item of parsed) {
      if (!item || typeof item !== "object") continue;

      const candidate = item as Partial<Round32Assignment>;
      if (
    ...
    return new Map<number, Round32Assignment>();
  }
}

function buildRound32Slots(
  existingRows: MatchRow[],
  assignments: Map<number, Round32Assignment>
): Round32Slot[] {
  ...
  return round32MatchNumbers.map((matchNumber, index) => {
    const fixture = getRealRound32Fixture(index);
...
function getAvailableTeamsForSlot(slots: Round32Slot[], slot: Round32Slot) {
...
async function saveRound32Assignments(formData: FormData) {
  "use server";
  ...
  if (!profile || !isAdmin(profile)) {
    redirectWithError("Accès administration refusé.");
  }

  const payloads = round32MatchNumbers.flatMap<Round32Write>((matchNumber, index) => {
    ...
    if (!fixture) {
      redirectWithError(`Impossible de retrouver le calendrier du match ${matchNumber}.`);
    }
  ...
  const { data: existingRows, error: existingRowsError } = await adminSupabase
    ...
    .eq("phase", realRound32Phase)
    .in("match_number", round32MatchNumbers);
  ...
  const { error: settingsError } = await adminSupabase
    ...
    .upsert(
      {
        key: round32AssignmentsSettingKey,
        ...
        <header className="overflow-hidden rounded-[2rem] border border-slate-200 bg-white/90 p-6 shadow-[0_24px_80px_rgba(15,23,42,0.08)] backdrop-blur sm:p-8">
          <div className="flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between">
            <div className="max-w-3xl space-y-4">
              ...
              <h1 className="text-4xl font-semibold tracking-tight text-slate-950 sm:text-5xl">
                Saisie des 16e réels
              </h1>
              <p className="max-w-2xl text-base leading-7 text-slate-600 sm:text-lg">
                Choisis la bonne équipe dans le bon seizième. Les 48 équipes sont proposées dans
                chaque liste déroulante, puis enregistrées comme matchs réels du tableau de 16e.
              </p>
            </div>
          ...
          <div className="mt-6 grid gap-3 sm:grid-cols-3">
            <div className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3">
              ...
              <p className="mt-2 text-2xl font-semibold text-slate-950">48</p>
            </div>
            <div className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3">
              <p className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">
                Seizièmes à remplir
              </p>
              <p className="mt-2 text-2xl font-semibold text-slate-950">16</p>
            </div>
            <div className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3">
              <p className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">
                Matchs prévus
              </p>
              <p className="mt-2 text-2xl font-semibold text-slate-950">73 - 88</p>
            </div>
          </div>
        </header>

        <form
          action={saveRound32Assignments}
          className="space-y-4"
        ...
        >
            ...
            >
              <div className="flex flex-col gap-3 border-b border-slate-100 pb-4 sm:flex-row sm:items-start sm:justify-between">
                ...
                <p className="max-w-xl text-sm leading-6 text-slate-500">
                  Les sélections doivent rester uniques sur l&apos;ensemble des 16 matchs.
                </p>
              </div>
          ...
          <div className="flex flex-col items-start justify-between gap-4 rounded-[1.75rem] border border-slate-200 bg-white p-5 shadow-[0_16px_40px_rgba(15,23,42,0.06)] lg:flex-row lg:items-center">
            <p className="text-sm leading-6 text-slate-600">
              Après sauvegarde, ces 16e sont réinjectés dans la table des matchs et réapparaissent
              dans la page publique du 2e tour réel.
            </p>
            <button
              type="submit"
              className="rounded-full bg-[#7a1f2c] px-6 py-3 text-sm font-semibold text-white shadow-sm transition hover:bg-[#5f1822]"
            >
              Enregistrer les 16 seizièmes
            </button>
          </div>
        </form>
      </div>
